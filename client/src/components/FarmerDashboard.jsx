  ...
  useEffect(() => {
	...
	const handleUpdateCrop = async () => {
		try {
			...
			if (response.data.success) {
				setCrops(crops.map((crop) => (crop._id === editCrop._id ? editCrop : crop)));
				setShowModal(false);
			} else {
						...
						<div className="space-y-4">
							{crops.map((crop) => (
								<div
									...
									} rounded-lg shadow flex items-center gap-4`}
								>
									...
									<div>
										...
										<p>
											<strong>Price Per Kg:</strong> ₹{crop.price}
										</p>
										<p>
...
<section className={`p-6 rounded-lg shadow-md mb-6 ${isDarkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
	...
	{loading ? (
		...
		<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
			{crops.map((crop) => (
				<div key={crop._id} className={`p-4 ${isDarkMode ? "bg-gray-800" : "bg-gray-100"} rounded-lg shadow-lg`}>
					{/* Crop Image */}
					<div className="relative">
						<img
							src={crop.image || "https://icarda.org/sites/default/files/styles/d11_standard/public/images/2018-10/14177187055_41e65491fa_o.webp?h=16013371&itok=1WYmBZGr"}
							alt={crop.name}
					...
					<div className="mt-4 space-y-2">
						...
						<p className="text-sm"><strong>Quantity:</strong> {crop.quantity} Kg</p>
						<p className="text-sm"><strong>Price Per Kg:</strong> ₹{crop.price}</p>
						<p className="text-sm"><strong>Description:</strong> {crop.description}</p>
					</div>
	...
	{/* Update Crop Modal */}
	{showModal && editCrop && (
		<div className={`fixed inset-0 flex items-center justify-center bg-opacity-50 ${isDarkMode ? "bg-black" : "bg-gray-800"}`}>
    <div className={`p-6 rounded-lg w-96 shadow-xl ${isDarkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"}`}>
      <h2 className="text-xl font-semibold mb-4 text-center">Update Crop</h2>
      ...
      <input 
        ...
        className={`border p-2 w-full rounded ${isDarkMode ? "bg-gray-800 border-gray-600 text-white" : "bg-white border-gray-300 text-black"}`} 
      />
  
      <label className="block mt-2 text-sm font-medium">Quantity (Kg):</label>
      <input 
        type="number" 
        name="quantity" 
      ...
      <label className="block mt-2 text-sm font-medium">Price Per Kg:</label>
      <input 
        type="number" 
        name="price" 
