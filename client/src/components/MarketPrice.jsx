...
const MarketPrice = () => {
  ...
  const statesList = [
    ...
    "Lakshadweep", "Puducherry", "Andaman and Nicobar Islands", "Chandigarh"
  ];
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <h1 className="text-4xl font-bold text-green-700 text-center mb-6">Live Market Prices</h1>
      ...
      <div className="flex justify-between mb-6">
        ...
        <div className="flex-2 ml-4">
          <label htmlFor="state-select" className="mr-4 font-medium">Filter by State:</label>
          <select
            id="state-select"
            className="p-2 border rounded w-full"
            value={selectedState}
          ...
          {filteredPrices.length > 0 ? (
            filteredPrices.map((price, index) => (
              <div key={index} className="border rounded-lg overflow-hidden shadow-lg bg-white">
                <div className="p-4">
                  <h2 className="text-xl font-bold text-green-700">{price.name}</h2>
                  <p className="text-gray-700 mt-2">Price: {price.price}</p>
                  <p className="text-sm text-gray-500 mt-2">Region: {price.region}</p>
                  <p className="text-sm text-gray-500 mt-2">Last Updated: {new Date(price.timestamp).toLocaleString()}</p>
                </div>
              </div>
            ))
          ) : (
